...
# G3パラメータ：ランダムウォーク汚染
WALK_STEP_SIZE = 0.01  # ランダムウォークのステップサイズ（信号の標準偏差の倍数）
...
# G3. ランダムウォーク汚染
print("\n[G3] Generating Random Walk Contamination...")
# 信号の標準偏差を計算
signal_std = np.std(original_signal)

# ランダムウォークを生成
steps = np.random.randn(N) * WALK_STEP_SIZE * signal_std
random_walk = np.cumsum(steps)

# ランダムウォークを追加
walk_signal = original_signal + random_walk

# ノイズ成分を抽出
walk_noise = walk_signal - original_signal

scale = calculate_noise_scale(original_signal, walk_noise, TARGET_SNR_DB)
walk_noise_scaled = walk_noise * scale
noisy_G3 = original_signal + walk_noise_scaled

print(f"  Walk step size: {WALK_STEP_SIZE * signal_std:.4e} V")
print(f"  Walk range: {random_walk.min():.4e} to {random_walk.max():.4e} V")
...
df_G3.to_csv('1_sample_G3_random_walk.csv', index=False)
print("  Saved: 1_sample_G3_random_walk.csv")
...
# 下段：ノイズ付き
ax2.plot(time_points, noisy_G3, linewidth=0.7, color='red', alpha=0.8)
ax2.set_xlabel('Time (sampling points)', fontsize=14)
ax2.set_ylabel('Voltage', fontsize=14)
ax2.set_title('Noisy Signal (Random Walk Contamination)', fontsize=12, fontweight='bold')
...
plt.tight_layout()
plt.savefig('1_sample_G3_random_walk.png', dpi=150, bbox_inches='tight')
plt.close()
print("  Saved: 1_sample_G3_random_walk.png")
