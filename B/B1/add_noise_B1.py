...
# B1パラメータ：インパルスノイズ
IMPULSE_PROBABILITY = 0.01  # 1%の確率でスパイク発生
IMPULSE_AMPLITUDE = 10.0  # スパイクの振幅倍率
...
# B1. インパルス（スパース）ノイズ
print("\n[B1] Generating Impulse (Salt & Pepper) Noise...")
# ランダムな位置でスパイク発生
impulse_mask = np.random.rand(N) < IMPULSE_PROBABILITY
impulse_noise = np.zeros(N)
# スパイク位置にランダムな正負の大振幅を付与
impulse_noise[impulse_mask] = np.random.choice([-1, 1], size=np.sum(impulse_mask)) * IMPULSE_AMPLITUDE

# 信号の標準偏差を基準にスケーリング
signal_std = np.std(original_signal)
impulse_noise_scaled = impulse_noise * signal_std

# SNRに基づいて調整
scale = calculate_noise_scale(original_signal, impulse_noise_scaled, TARGET_SNR_DB)
impulse_noise_scaled = impulse_noise_scaled * scale

noisy_B1 = original_signal + impulse_noise_scaled

print(f"  Impulses generated: {np.sum(impulse_mask)} / {N} points")
...
df_B1.to_csv('1_sample_B1_impulse_salt_pepper.csv', index=False)
print("  Saved: 1_sample_B1_impulse_salt_pepper.csv")
...
# 下段：ノイズ付き
ax2.plot(time_points, noisy_B1, linewidth=0.7, color='red', alpha=0.8)
ax2.set_xlabel('Time (sampling points)', fontsize=14)
ax2.set_ylabel('Voltage', fontsize=14)
ax2.set_title('Noisy Signal (Impulse/Salt & Pepper)', fontsize=12, fontweight='bold')
...
plt.tight_layout()
plt.savefig('1_sample_B1_impulse_salt_pepper.png', dpi=150, bbox_inches='tight')
plt.close()
print("  Saved: 1_sample_B1_impulse_salt_pepper.png")
