...
# F2パラメータ：ランダム時間マスク
MASK_DURATION = 20  # 各マスクの長さ（サンプル）
MASK_PROBABILITY = 0.02  # マスク発生確率（2%）
...
# F2. ランダム時間マスク
print("\n[F2] Generating Random Time Mask...")
masked_signal = original_signal.copy()

# ランダムにマスク開始位置を選択
num_masks = 0
i = 0
while i < N - MASK_DURATION:
    if np.random.rand() < MASK_PROBABILITY:
        # マスク区間をゼロに設定
        masked_signal[i:i+MASK_DURATION] = 0
        num_masks += 1
        i += MASK_DURATION  # マスク後は次の位置へ
    else:
        i += 1

# ノイズ成分を抽出
mask_noise = masked_signal - original_signal

scale = calculate_noise_scale(original_signal, mask_noise, TARGET_SNR_DB)
mask_noise_scaled = mask_noise * scale
noisy_F2 = original_signal + mask_noise_scaled

print(f"  Mask duration: {MASK_DURATION} samples")
print(f"  Number of masks applied: {num_masks}")
...
df_F2.to_csv('1_sample_F2_random_time_mask.csv', index=False)
print("  Saved: 1_sample_F2_random_time_mask.csv")
...
# 下段：ノイズ付き
ax2.plot(time_points, noisy_F2, linewidth=0.7, color='red', alpha=0.8)
ax2.set_xlabel('Time (sampling points)', fontsize=14)
ax2.set_ylabel('Voltage', fontsize=14)
ax2.set_title('Noisy Signal (Random Time Mask)', fontsize=12, fontweight='bold')
...
plt.tight_layout()
plt.savefig('1_sample_F2_random_time_mask.png', dpi=150, bbox_inches='tight')
plt.close()
print("  Saved: 1_sample_F2_random_time_mask.png")
