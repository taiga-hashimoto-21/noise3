...
# E2パラメータ：ベースラインドリフト
DRIFT_AMPLITUDE = 0.5  # ドリフトの振幅（信号の標準偏差の倍数）
DRIFT_ORDER = 2  # 多項式の次数（2次=放物線）
...
# E2. ベースラインドリフト
print("\n[E2] Generating Baseline Drift...")
# 正規化された時間軸（0から1）
t_norm = (time_points - time_points[0]) / (time_points[-1] - time_points[0])

# 2次多項式ドリフト（放物線）
signal_std = np.std(original_signal)
baseline_drift = DRIFT_AMPLITUDE * signal_std * (t_norm ** DRIFT_ORDER - 0.5)

# ドリフトを追加
drift_signal = original_signal + baseline_drift

# ノイズ成分を抽出
drift_noise = drift_signal - original_signal

scale = calculate_noise_scale(original_signal, drift_noise, TARGET_SNR_DB)
drift_noise_scaled = drift_noise * scale
noisy_E2 = original_signal + drift_noise_scaled

print(f"  Drift range: {baseline_drift.min():.4e} to {baseline_drift.max():.4e} V")
...
df_E2.to_csv('1_sample_E2_baseline_drift.csv', index=False)
print("  Saved: 1_sample_E2_baseline_drift.csv")
...
# 下段：ノイズ付き
ax2.plot(time_points, noisy_E2, linewidth=0.7, color='red', alpha=0.8)
ax2.set_xlabel('Time (sampling points)', fontsize=14)
ax2.set_ylabel('Voltage', fontsize=14)
ax2.set_title('Noisy Signal (Baseline Drift)', fontsize=12, fontweight='bold')
...
plt.tight_layout()
plt.savefig('1_sample_E2_baseline_drift.png', dpi=150, bbox_inches='tight')
plt.close()
print("  Saved: 1_sample_E2_baseline_drift.png")
